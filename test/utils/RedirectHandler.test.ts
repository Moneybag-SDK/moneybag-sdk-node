import { RedirectHandler } from '../../src/utils/RedirectHandler';
...
describe('RedirectHandler', () => {
  describe('parseRedirectUrl', () => {
    it('should parse full redirect URL with query parameters', () => {
      ...
      const result = RedirectHandler.parseRedirectUrl(url);
    ...
    it('should parse query string only', () => {
      ...
      const result = RedirectHandler.parseRedirectUrl(queryString);
    ...
    it('should throw error for missing transaction_id', () => {
      ...
      expect(() => RedirectHandler.parseRedirectUrl(url)).toThrow(
        'Invalid redirect URL: missing required parameters'
      );
    });
  ...
  describe('buildRedirectUrl', () => {
    it('should build redirect URL with SUCCESS status', () => {
      ...
      const result = RedirectHandler.buildRedirectUrl(baseUrl, params);
  ...
  describe('getRedirectUrlByStatus', () => {
    ...
    it('should return success URL for SUCCESS status', () => {
      const result = RedirectHandler.getRedirectUrlByStatus('SUCCESS', urls);
      expect(result).toBe('https://example.com/success');
    });

    it('should return fail URL for FAILED status', () => {
      const result = RedirectHandler.getRedirectUrlByStatus('FAILED', urls);
      expect(result).toBe('https://example.com/fail');
    });

    it('should return cancel URL for CANCELLED status', () => {
      const result = RedirectHandler.getRedirectUrlByStatus('CANCELLED', urls);
      expect(result).toBe('https://example.com/cancel');
    });

    it('should return fail URL for PENDING status', () => {
      const result = RedirectHandler.getRedirectUrlByStatus('PENDING', urls);
      expect(result).toBe('https://example.com/fail');
    });

    it('should return fail URL for unknown status', () => {
      const result = RedirectHandler.getRedirectUrlByStatus('UNKNOWN' as any, urls);
      expect(result).toBe('https://example.com/fail');
    });
  });
});